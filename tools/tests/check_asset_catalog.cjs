...
(async () => {
  ...
  try {
    ...
    await page.locator('#search').fill('Moblin');
    await page.getByRole('button',{name:'Inspect spr_Moblin_High_DrawA',exact:true}).click();
    await page.screenshot({path:path.join(qa,'catalog-mobile.png'),fullPage:true});
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth),false);
    assert.deepEqual(errors,[]);
    console.log('PASS: file:// load, search, type/group filters, pagination, inspector, tiles, playback, selection, brief/PNG downloads, mobile layout; no JS errors.');
